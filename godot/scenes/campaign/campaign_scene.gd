...
func _draw() -> void:
	...
	for row in _row_rects():
		...
		draw_string(fnt, Vector2(r.position.x + 18, r.position.y + 42), teaser, HORIZONTAL_ALIGNMENT_LEFT, -1, 10, Pal.INK_DIM if unlocked else Pal.INK_FAINT)
		...
		draw_string(fnt, Vector2(r.position.x + r.size.x - 120, r.position.y + 22), badge, HORIZONTAL_ALIGNMENT_RIGHT, 104, 11, bcol)
		draw_string(fnt, Vector2(r.position.x + r.size.x - 120, r.position.y + 42), String(sc["difficulty"]).to_upper(), HORIZONTAL_ALIGNMENT_RIGHT, 104, 10, Pal.INK_DIM if unlocked else Pal.INK_FAINT)
